...
;;lazy lety
;;wariant pierwszy, gdzie  w srodowisku przechowuje domkniecia
;;eval dla tego predykatu jest dokladnie taki sam jak dla zwyklych letow
;;tylko wywoluje odpowiednie funkcje leniwe
...
;;rozpatrywanie lazy-leta, wrzuca zmienna i bezargumentowe domkniecie dla tej zmiennej
(define (env-for-lazy-let def env)
  (add-to-env
    (let-def-var def)
    (closure-cons '() (let-def-expr def) env)
    env))

;;zmieniona procedura szukania w srodowisku jesli znalezlismy zmienna, to patrzymy czy
;;jest zwiazana domknieciem, jak tak to evalujemy to domkniecie zgodnie z jego wlasnym srodowiskiem
(define (find-in-env x env)
  (cond [(null? env) (error "undefined variable" x)]
        [(eq? x (caar env))
         (let ((def (cadar env)))
         (if (closure? def)
                  (eval-env  (closure-expr def) (closure-env def))
                  def))]
        [else (find-in-env x (cdr env))]))


;;silnia
(eval '(let (silnia (lambda-rec (fact n)
            (lazy-let (t 1)
            (lazy-let (f (* n (fact (- n 1))))
                      (if (= n 0) t f)))))
         (silnia 7)))

;;zachowanie statycznego wiazania
(eval '(let (x 4)
         (lazy-let (y (+ x 1))
                   (let (x 10)
                     y))))

(eval '(lazy-let (x (+ 7 35))
                    (lazy-let (x (* x 2))
                    x)))
...
;zagniezdzone srodowiska
(eval '(lazy-let (y (+ 4 6))
                 (lazy-let (x (- y 10))
                           (lazy-let (z (* x y))
                                     (lambda (x) (* x z))))))

;;lista natsow od danej liczby
(eval '(let (naturals (lambda-rec (nats n)
                                  (lazy-let (f (nats (- n 1))) (if (= n 0) null (cons n f)))))
         (naturals 13)))
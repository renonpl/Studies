...
(define (solve-ax+by=1 a b)
  (define (solver p)                                   ;;bierzemy pare, ktora ma miec rozwiazanie ax+by=1
    (let ((a (car p)) (b (cdr p)))                    ;; gdzie NWD(a,b) = 1
        (if (= b 0) p                                  ;;jak b = 0 to wracamy
            (let ((z (solver (cons b (remainder a b)))))  ;;niech z bedzie kolejnym wywolaniem solver - w dol przekazujemy kolejne a i b, a dostajemy odpowiednie x i y
            (cons (cdr z)(- (car z) (* (cdr z) (quotient a b)))))))) ;;zwracamy rozwiazanie tej rownosci zgodnie ze wzorem wyzej
  (solver (cons a b)))
...
#|
W procedurze RSA_conver-list z ciagu liczb i1 i2 i3 i4 otrzymujemy kolejno liczby tr(i1) tr(i2-tr(i1) tr(i3 - tr(i2 -tr(i1))) itd.
Zeby odwrocic ta operacje musimy na kazdym elemencie przeprowadzic powownie transformacje, a potem dodac poprzedni element ciagu,
czyli dla tr(i1) wystarczy tylko transformacja, a dla tr(i2-tr(i1) po trans uzyskujemy i2-tr(i1) a wiec musimy dodac tr(i1).
Ogolnie dla rozszyfrowania n'tego elementu ciagu musimy zapamietac n-1 element.
Oczywiscie wszystkie operacje mamy w swiecie modulo n.
W jedna strone do transformacji uzywamy klucza publicznego a w druga prywatnego
|#

(define (RSA-unconvert-list intlist key)
  (let ((n (key-modulus key)))
        (define (unconvert list last_el)
          (if (null? list)
              '()
              (let ((x (modulo (+ (RSA-transform (car list) key) last_el) n )))
                (cons x (unconvert (cdr list) (car list))))))
  (unconvert intlist 0)))
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
    (let ((encrypted (RSA-encrypt message recipient-public-key)))    ;;kodujemy wiadomosc kluczem odbiorcy
      (let ((hasz (compress encrypted)))    ;;haszujemy zakodowana wiadomosc
        (make-signed-message (RSA-transform hasz sender-private-key) encrypted))))  ;;szyfrujemy zahaszowany podpisy naszym kluczem prywatnym

(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  (let ((hasz (compress (message cyphertext)))   ;;haszujemy wiadomosc
        (signature (RSA-transform (signature cyphertext) sender-public-key)))  ;;odszyfrujemy podpis
    (if (= hasz signature)
        (RSA-decrypt (message cyphertext) recipient-private-key) ;;odszyfrujemy wiadomosc
...
;;testy 1
(RSA-decrypt (RSA-encrypt "hohohoho" (key-pair-public test-key-pair1)) (key-pair-private test-key-pair1))
(string->intlist "testytestujasie")
(RSA-encrypt "testytestujasie" (key-pair-public test-key-pair2))
(RSA-unconvert-list '(376594566 405895905 491545798 119312161) (key-pair-private test-key-pair2))
(RSA-decrypt (RSA-encrypt "klucznumerouno" (key-pair-public test-key-pair1)) (key-pair-private test-key-pair1)) ;;spacje na koncu nie sa bledem
(RSA-decrypt (RSA-encrypt "testowaniekluczanumerodos" (key-pair-public test-key-pair2)) (key-pair-private test-key-pair2)) ;;spacje na koncu nie sa bledem


;;test2
(solve-ax+by=1 39 14)
(solve-ax+by=1 42 13)
(solve-ax+by=1 38977 21701)
(solve-ax+by=1 1 0)
(solve-ax+by=1 14 39)
...
(generate-RSA-key-pair)
;;testy3
(RSA-decrypt (RSA-encrypt "szyfrowackazdymoze" (key-pair-public test-key-pair2)) (key-pair-private test-key-pair2)) ;;spacje na koncu nie sa bledem
(RSA-decrypt (RSA-encrypt "jeden lepiej drugi troche gorzej" (key-pair-public test-key-pair2)) (key-pair-private test-key-pair2))

(define secret (encrypt-and-sign "Sekretna Wiadomosc do Prezydenta" (key-pair-public test-key-pair1) (key-pair-private test-key-pair2)))
(authenticate-and-decrypt secret (key-pair-private test-key-pair1) (key-pair-public test-key-pair2))

;;tu blad
(authenticate-and-decrypt (make-signed-message 42  '(376594566 405895905 491545798 119312161)) (key-pair-private test-key-pair1) (key-pair-public test-key-pair2))